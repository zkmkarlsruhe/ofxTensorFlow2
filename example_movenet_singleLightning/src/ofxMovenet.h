...
public:
    ...
    static const int NUM_SKELETONS = 1; //6; //< max skeletons tracked
    static const int DATA_PER_SKELETON = 51; //56; //< output values per skeleton, Detected landmark coordinates and scores of shape (N, 17 * 3)
    ...
    struct Skeleton {
        ...
        float bone_confidenceThreshold;
        
        /// draw a line between two bone points
        void drawBone(BoneIndex b1, BoneIndex b2) {
            if(bones[b1].confidence < bone_confidenceThreshold) return;
            ofMesh m;
    ...
    void draw() {
        ...
        int tempY = 10;
        ofDrawBitmapString("use mouse to adjust confidence thresholds", 200, tempY+=15);
        tempY +=5;
        
        float confidenceThreshold = ofMap(ofGetMouseX(),0,ofGetWidth(),0,1,true);
        
        for(auto skeleton : skeletons) {
            ofNoFill();
            ofSetColor(255,255);
            ofDrawRectangle(skeleton.bbox);
            skeleton.bone_confidenceThreshold = ofMap(ofGetMouseY(),0,ofGetHeight(),0,1,true);
            if(skeleton.confidence > confidenceThreshold) {
                skeleton.draw();
            }
            
           
            ofDrawBitmapString("skeleton confidence: "+ofToString(skeleton.confidence,4) + " , thresh " + ofToString(confidenceThreshold,4), 200, tempY+=15);
            ofDrawBitmapString("bone thresh: "+ ofToString(skeleton.bone_confidenceThreshold,4), 200, tempY+=15);
            
            tempY += 15;
            
            ofSetColor(255,ofMap(ofGetMouseX(),0,ofGetWidth(),255,0,true));
            if(skeleton.confidence > ofMap(ofGetMouseX(),0,ofGetWidth(),0,1,true)) {
                ofSetColor(255,255);
            }
            for(auto & aBone : skeleton.bones) {
                ofDrawBitmapString(ofToString(aBone.confidence,4), 200, tempY+=15);
            }
        }
...
//        }
        ...
        for(int i = 0; i < NUM_SKELETONS; i++) {
            ...
            skeleton.bbox.width = xmax - xmin;
            skeleton.bbox.height = ymax - ymin;
            skeleton.confidence = avgConfidence/float(BONES_PER_SKELETON); //vectorOut[d+55];
