...
std::vector<std::vector<uint8_t>> gpuMemoryGrowPresets{
...
// serialized config options whitout memory growth
std::vector<std::vector<uint8_t>> gpuMemoryNoGrowPresets{
...
void setTFgpuRAMreservation(GpuMemoryFraction fraction, bool growth){
	...
	if (growth){
		config = gpuMemoryGrowPresets[fraction];
	}
	else {
		config = gpuMemoryNoGrowPresets[fraction];
	}
